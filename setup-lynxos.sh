...
# ── Установить adw-gtk3 из AUR ──────────────
echo "Установка adw-gtk3 (AUR)..."
if ! pacman -Qi adw-gtk3 &>/dev/null; then
    # Сборка от временного пользователя
    useradd -m -s /bin/bash _builduser 2>/dev/null || true
    echo "_builduser ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/builduser
    cd /tmp
    sudo -u _builduser git clone --depth=1 \
        https://aur.archlinux.org/adw-gtk3.git 2>/dev/null || true
    if [ -d /tmp/adw-gtk3 ]; then
        cd /tmp/adw-gtk3
        sudo -u _builduser makepkg -si --noconfirm 2>/dev/null && \
            echo "✅ adw-gtk3 установлен" || \
            echo "⚠️  adw-gtk3: пропущено (без интернета)"
    fi
...
# ── Установить LocalSend из AUR ─────────────
echo "Установка LocalSend (AUR)..."
useradd -m -s /bin/bash _builduser2 2>/dev/null || true
echo "_builduser2 ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/builduser2
cd /tmp
sudo -u _builduser2 git clone --depth=1 \
    https://aur.archlinux.org/localsend-bin.git 2>/dev/null || true
if [ -d /tmp/localsend-bin ]; then
    cd /tmp/localsend-bin
    sudo -u _builduser2 makepkg -si --noconfirm 2>/dev/null && \
        echo "✅ LocalSend установлен" || \
        echo "⚠️  LocalSend: пропущено"
fi
userdel -r _builduser2 2>/dev/null || true
rm -f /etc/sudoers.d/builduser2
cd /

# ── Установить VS Code из AUR ───────────────
echo "Установка Visual Studio Code (AUR)..."
useradd -m -s /bin/bash _builduser3 2>/dev/null || true
echo "_builduser3 ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/builduser3
cd /tmp
sudo -u _builduser3 git clone --depth=1 \
    https://aur.archlinux.org/visual-studio-code-bin.git 2>/dev/null || true
if [ -d /tmp/visual-studio-code-bin ]; then
    cd /tmp/visual-studio-code-bin
    sudo -u _builduser3 makepkg -si --noconfirm 2>/dev/null && \
        echo "✅ VS Code установлен" || \
        echo "⚠️  VS Code: пропущено (без интернета или долгая загрузка)"
fi
userdel -r _builduser3 2>/dev/null || true
rm -f /etc/sudoers.d/builduser3
cd /

# ── Установить yay (AUR helper) ─────────────
echo "Установка yay..."
if ! command -v yay &>/dev/null; then
    useradd -m -s /bin/bash _yaybuilder 2>/dev/null || true
    echo "_yaybuilder ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers.d/yaybuilder
    cd /tmp
    sudo -u _yaybuilder git clone --depth=1 \
        https://aur.archlinux.org/yay.git 2>/dev/null || true
    if [ -d /tmp/yay ]; then
        cd /tmp/yay
        sudo -u _yaybuilder makepkg -si --noconfirm 2>/dev/null && \
            echo "✅ yay установлен" || \
            echo "⚠️  yay: пропущено"
    fi
    userdel -r _yaybuilder 2>/dev/null || true
...
# ── Очистка ─────────────────────────────────
rm -rf /tmp/adw-gtk3 /tmp/localsend-bin /tmp/visual-studio-code-bin /tmp/yay
